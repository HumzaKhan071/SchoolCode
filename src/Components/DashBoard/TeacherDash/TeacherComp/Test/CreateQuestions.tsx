...
const CreateQuestions = () => {
  ...
  const onSubmit = handleSubmit(async (data: any) => {
    ...
    await axios.post(`${url}/api/test/${id}/creating-option`, data);

    reset();
  });

  const [dataFile, setDataFile] = React.useState({} as any);
  const [dataOption, setDataOption] = React.useState([] as any[]);
  ...
  const fetchDataII = async () => {
    ...
    await axios(newURL).then((res: any) => {
      setDataOption(res?.data?.data?.mainTest);
    });
  };
      ...
      <Wrapper>
        <TestBox>
          ...
          <TypedQuestion>
            <TypedQuestionHold>
              <MainQuestions>
                {dataOption?.map((props: any, i: any) => (
